...
class ConversationService:
    ...
    def chat(
        ...
        if conversation_id is None:
            ...
            conversation = self.get_conversation(db=db, conversation_id=conversation_id)
            if conversation is None:
                logger.error(f"Conversation not found: {conversation_id}")
                raise ValueError(f"Conversation with id {conversation_id} not found")
        ...
        try:
            ...
            user_message = self.message_repository.create_without_commit(
                db=db,
                conversation_id=conversation_id,
                role="user",
                content=question,
            ...
            assistant_message = self.message_repository.create_without_commit(
                db=db,
                conversation_id=conversation_id,
                role="assistant",
                content=answer,
